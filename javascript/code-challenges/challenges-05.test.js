...
const returnNames = (arr) => {
  return arr.reduce((newArr, currentPerson) => {
    newArr.push(currentPerson.name);
    return newArr;
  }, []);
};
...
const calculateAverage = (arr) => {
  let obj = arr.reduce((accumulator, num) => {
    accumulator.count += 1;
    accumulator.sum += num;
    return {count: accumulator.count, sum: accumulator.sum};
  }, {count: 0, sum: 0});
